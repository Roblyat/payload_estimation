...
from preprocess_delan.dataset import normalize_out_npz_path, dataset_json_path
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Derive acceleration qdd from velocity qd via numerical differentiation.",
    )
    ...
    ap.add_argument("--out_npz", type=str,
        default=os.environ.get("OUT_NPZ", "/workspace/shared/data/preprocessed/delan_ur5_dataset/delan_ur5_dataset.npz"),
    ...
    derive_qdd = args.derive_qdd_from_qd
    ...
    cfg = DelanPreprocessConfig(
        ...
        filter_cutoff_hz=args.lowpass_cutoff_hz,
        filter_order=args.lowpass_order,
        filter_qdd=args.lowpass_qdd,
    )

    raw_csv = args.raw_csv 
    out_npz = args.out_npz

    # If user passes a directory, auto-name the file inside it.
    if os.path.isdir(out_npz):
        ds_stem = Path(raw_csv).stem
        out_npz = os.path.join(out_npz, f"delan_{ds_stem}_dataset.npz")

    out_npz = normalize_out_npz_path(out_npz)
